...
describe('CollectionTest', () => {
    ...
    describe('testFind', () => {
        ...
        it('Should find and filter data', () => {
            assert.equal(1, collection.findAll({
                where: {
                    version: '7.13.1'
                }
            }).length);

            assert.equal('Google', collection.find({
                where: {
                    code: { neq: 10 }
                }
            }).name);

            assert.equal(2, collection.findAll({
                where: {
                    name: { like: 'oo' }
                }
            }).length);

            assert.isNull(collection.find({
                where: {
                    code: { gte: 10 }
                }
            }));
    ...
    describe('testDeletingData', () => {
        ...
        it('Should delete data based on filters', () => {
            ...
            assert.isTrue(collection.delete({
                where: {
                    code: { gt: 1 }
                }
            }, true));
